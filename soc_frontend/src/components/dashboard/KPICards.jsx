...
import { AlertOctagon, ShieldAlert, Radio, SearchCode, Calendar, ShieldCheck } from 'lucide-react';

export function KPICards({ kpis, incidents }) {
  const activeCritical = incidents?.filter(i => i.severity === 'CRITICAL' && i.status !== 'RESOLVED' && i.status !== 'FALSE_POSITIVE').length ?? kpis.activeCritical;
  const highRisk = incidents?.filter(i => (i.severity === 'HIGH' || i.riskScore >= 70) && i.status !== 'RESOLVED' && i.status !== 'FALSE_POSITIVE').length ?? kpis.highRiskEvents;
  const openInvestigations = incidents?.filter(i => i.status === 'UNDER_INVESTIGATION' || i.status === 'OPEN').length ?? kpis.openInvestigations;
  ...
  const cards = [
    ...
    },
    {
      ...
      value: incidents?.length || kpis?.incidentsToday || 14,
      trend: '24h aggregate',
    ...
    },
    {
      ...
      value: kpis?.protectedIdentities || 18,
      trend: '100% enrolled',
