...
export default function SecuritySettingsPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-6 space-y-5">
      ...
      <Card header={<span className="text-sm font-semibold text-slate-700">Security Warning Language</span>}>
        <p className="text-xs text-slate-500 mb-4">Choose the language for real-time security warnings and alerts.</p>
        <div className="space-y-3">
          {[
            ...
            <label key={lang.id} className="flex items-center justify-between p-3 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
              <div className="flex items-center gap-3">
                <input
                  type="radio"
                  name="warningLanguage"
                  value={lang.id}
                  checked={settings.warningLanguage === lang.id}
                  onChange={() => updateSettings({ warningLanguage: lang.id as any })}
                  className="w-4 h-4 text-blue-600 focus:ring-blue-500"
                />
                <div>
                  <div className="text-sm font-medium text-slate-900">{lang.native}</div>
                  <div className="text-xs text-slate-500">{lang.label}</div>
                </div>
              </div>
              {settings.warningLanguage === lang.id && (
                <span className="text-xs font-medium text-blue-700 bg-blue-100 px-2 py-1 rounded-md">Active</span>
              )}
            </label>
          ))}
        </div>
