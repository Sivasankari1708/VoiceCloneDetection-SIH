  ...
  async verifyOtp(usernameInput: string, passwordOrOtp: string): Promise<{ token: string; user: User }> {
    ...
    if (!res.ok) {
      if (res.status === 401) {
        ...
        try {
          const regRes = await fetch(`${config.apiBaseUrl}/api/auth/register`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              username,
              ...
              role: username.toLowerCase().includes('attacker') ? 'ATTACKER' : 'USER'
            }),
            signal: AbortSignal.timeout(5000),
          });
          if (regRes.ok) {
          ...
          throw new Error(`Login failed (HTTP ${res.status}) and auto-registration failed.`);
        }
      } else {
