...
function mapBackendCallSummary(raw: BackendCallSummary): CallHistoryItem {
  ...
  const endTime = raw.end_time ? new Date(raw.end_time) : new Date();
  const duration = Math.max(0, Math.floor((endTime.getTime() - startTime.getTime()) / 1000));
  const severity: SeverityLevel = raw.current_risk_level
    ? mapRiskLevel(raw.current_risk_level)
    : 'SAFE';
  ...
  return {
    ...
    finalScore: raw.current_risk_score ?? 0,
