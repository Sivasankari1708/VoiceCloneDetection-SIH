  ...
  private connectWebSocket(sessionId: string, params: StartCallParams): void {
    ...
    ws.onmessage = (evt) => {
      ...
      try {
        ...
        } else if (eventName === 'VERIFICATION_EVENT') {
          // Batch 2B — verification lifecycle event from backend
          const vData = data as Record<string, unknown>;
          console.info(
            '[WS RX] VERIFICATION_EVENT | type=%s | outcome=%s | phrase=%s',
            vData?.verification_type ?? 'UNKNOWN',
            vData?.outcome ?? 'N/A',
            vData?.challenge_phrase ?? ''
          );
          this.emit({ type: 'verification_requested', timestamp: Date.now(), payload: { security: vData as any } });
        } else if (eventName === 'ERROR') {
          console.warn('[WS RX] Backend ERROR:', data);
        } else {
          // Unknown event — log safely, do not crash
          console.info('[WS RX] Unknown event received (ignored):', eventName);
        }
