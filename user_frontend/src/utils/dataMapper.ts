...
export function mapRiskLevel(level: string): SeverityLevel {
  const map: Record<string, SeverityLevel> = {
    ...
    LOW: 'LOW',
    CAUTION: 'CAUTION',
    MEDIUM: 'CAUTION', // Map MEDIUM to CAUTION per risk progression: SAFE, LOW, CAUTION, HIGH, CRITICAL
    HIGH: 'HIGH',
    CRITICAL: 'CRITICAL',
  };
...
export function mapIdentityStatus(value: string | number): IdentityStatus {
  if (typeof value === 'number') {
    ...
    if (value >= 0.45 && value < 0.65) return 'degraded';
  ...
  switch (value?.toUpperCase()) {
    ...
    case 'INCONCLUSIVE': return 'degraded';
...
export function identityStatusMessage(status: IdentityStatus): string {
  switch (status) {
    ...
    case 'degraded': return "We couldn't confidently verify the caller. Voice or call conditions may be affecting verification.";
...
export function mapBackendRiskUpdate(
  ...
  const security: Partial<SecurityStatus> = {
    ...
    recommendedAction: update.recommended_action // Expose raw recommended_action for Batch 2B policy logic
  };
